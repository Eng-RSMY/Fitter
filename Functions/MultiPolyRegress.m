...
%   Copyright (c) 2015, Ahmet Cecen  -  All rights reserved.
    ...
    for ii=1:NLegend
        ...
        for jj=1:length(currentTerm)
            if jj==1
                currentLegend=[currentLegend,'x',num2str(currentTerm(jj))]; %#ok<*AGROW>
    ...
    Scores = cell2mat(cellfun(@(currentData) prod(bsxfun(@(x,y) x.^y,currentData,A),2)',Data,'UniformOutput',false));
    ...
    reg = struct('FitParameters','-----------------','PowerMatrix',A,'Scores',Scores, ...
        ...
        'Normalization',NormalizationSwitch,'LOOCVGoodnessOfFit','-----------------', 'CVResiduals', rCV,'CVRSquare', ...
